...
\]
siendo $\sigma_F$ y $\sigma_r$ las incertidumbres en la fuerza y en la distancia, respectivamente. El error en el ángulo de giro no se ha incluido explícitamente en el ajuste lineal, pero se considera como una fuente adicional de dispersión en los datos y se tendrá en cuenta en la discusión de resultados.
...
\bigskip
Una vez determinada la constante recuperadora del muelle con un nivel de precisión satisfactorio, es posible avanzar hacia la siguiente fase del análisis. Con el valor de 

\[
\nu = (0.0326 \pm 0.0006)\ \text{N·m/rad},
\]

junto con los periodos medios de oscilación obtenidos para cada uno de los sólidos analizados, puede aplicarse la ecuación (4) para calcular sus momentos de inercia experimentales. Paralelamente, a partir de la masa y dimensiones geométricas de cada objeto, es posible determinar sus momentos de inercia teóricos mediante las expresiones correspondientes.
...
Una vez obtenidos los periodos medios de oscilación para cada uno de los cuerpos analizados, es posible avanzar hacia la determinación de sus momentos de inercia teóricos. Conociendo las dimensiones geométricas y las masas de cada objeto, y aplicando las expresiones correspondientes a su distribución de masa, podremos calcular los valores esperados de $I$ para la varilla, la esfera, el disco y el cilindro. Estos resultados servirán posteriormente para comparar de forma directa los valores teóricos con los momentos de inercia experimentales obtenidos a partir de los periodos, permitiendo evaluar la coherencia del modelo físico y la precisión de las medidas.
...
\bigskip
Al confrontar estos valores con los momentos de inercia derivados experimentalmente de los periodos medios de oscilación, se observa en todos los casos un grado de concordancia razonable. Las diferencias encontradas entre los valores teóricos y experimentales se sitúan dentro de los márgenes de error asociados tanto a las incertidumbres instrumentales como al ajuste realizado para la determinación de la constante del muelle. Esto sugiere que el comportamiento dinámico del sistema responde adecuadamente al modelo físico empleado y que los sólidos utilizados pueden describirse de forma satisfactoria mediante las expresiones teóricas estándar de su momento de inercia.

\bigskip
Las ligeras discrepancias presentes en algunos objetos pueden explicarse por diversas fuentes de error: pequeñas desviaciones geométricas respecto a los modelos ideales (por ejemplo, densidad no perfectamente uniforme o imperfecciones en la fabricación), errores de medición en las dimensiones o en la masa, y dispersiones en los periodos debidas a fricción interna en el sistema o a una posible no idealidad del muelle de torsión. A pesar de ello, el acuerdo general entre teoría y experimento confirma que el método empleado es fiable y que el modelo teórico describe correctamente la física del sistema.

\bigskip
En conjunto, los resultados permiten afirmar que los valores experimentales obtenidos para el momento de inercia se corresponden adecuadamente con los resultados teóricos, validando así tanto las expresiones utilizadas como la calidad de las mediciones realizadas.

\bigskip
Tras comparar los valores teóricos y experimentales de los momentos de inercia y comprobar la coherencia general entre ambos, podemos pasar ahora a la fase final del estudio, centrada en la verificación directa del teorema de Steiner. Para ello, analizaremos cómo varía el periodo de oscilación de la varilla cuando el eje de giro se desplaza a diferentes distancias de su centro de masas.

\subsection{Teorema de Steiner}

En esta última parte, se representará el cuadrado del periodo de oscilación en función del cuadrado de la distancia entre el eje de giro y el centro de la varilla. La teoría establece que dicha relación debe ser lineal, y que la pendiente del ajuste está vinculada a la constante recuperadora del muelle según la ecuación (6). De este modo, a partir de la recta obtenida, es posible extraer nuevamente el valor de la constante del muelle y evaluar si coincide con el determinado en el primer apartado de la práctica.
...
A partir de las representaciones gráficas de $T^2$ frente a $a^2$ para la varilla y para el disco, se procedió a realizar los ajustes lineales correspondientes, tal como establece la ecuación (6). Estos ajustes permiten extraer directamente la pendiente asociada a cada objeto, magnitud que está ligada a la constante recuperadora del muelle. A continuación se muestran los resultados numéricos obtenidos, junto con sus incertidumbres, que posteriormente serán comparados con la constante determinada en el primer apartado de la práctica.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\columnwidth]{imagenes/datos_steiner_varilla_disco}
    \caption{Datos experimentales utilizados para los ajustes de Steiner en la varilla y el disco.}
    \label{fig:datos_steiner_varilla_disco}
\end{figure}
...
\[
\nu_{\text{Steiner, varilla}} = 0.01836 \pm 0.00027.
\]

La linealidad del ajuste es elevada, lo que indica que la variación del periodo con la distancia al eje sigue adecuadamente el comportamiento predicho por el teorema de Steiner. Sin embargo, el valor de $\nu$ obtenido es sensiblemente menor que el calculado en la primera parte de la práctica 
\[
\nu = 0.0404 \pm 0.0007,
\]

Lo que apunta a la presencia de errores sistemáticos, probablemente asociados a la dificultad de fijar con precisión la posición del eje y al aumento de fricción para desplazamientos grandes del punto de giro.
...
\[
\nu_{\text{Steiner, disco}} = 0.03725 \pm 0.00175.
\]

En este caso, el valor de la constante recuperadora está mucho más próximo al determinado previamente, lo que indica que el disco se acopla al sistema de forma más estable y simétrica, reduciendo los errores sistemáticos relacionados con la alineación y la distribución de masa.

\bigskip
En conjunto, puede afirmarse que los datos experimentales verifican la linealidad predicha por el teorema de Steiner, ya que tanto para la varilla como para el disco se obtiene una dependencia claramente lineal entre $T^2$ y $a^2$. No obstante, la concordancia con la constante recuperadora obtenida inicialmente es desigual: el disco arroja un valor compatible dentro de los márgenes de error, mientras que la varilla presenta una discrepancia apreciable. Esto sugiere que el comportamiento del sistema es más sensible a errores geométricos cuando el eje de giro se desplaza respecto al centro en un sólido alargado como la varilla.

\bigskip
A pesar de estas diferencias, el análisis global permite concluir que el teorema de Steiner se verifica cualitativamente en ambos casos y que, especialmente para el disco, la determinación de $\nu$ mediante el ajuste lineal resulta coherente con la obtenida en la primera parte de la práctica.
